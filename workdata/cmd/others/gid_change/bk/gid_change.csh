...
#choice=0
#IFS=$'\n'
#menu=($(cat /workdata/XCSR/others/gid_change/tmp/menu.tmp))
#tail=`expr ${#menu[@]} - 1`
...
#        read -sn2 answer
#    elif [ "$answer" = "q" ]; then
#        echo "quit" > /workdata/XCSR/others/gid_change/tmp/answer.tmp
#        exit 0
#    fi
#    case $answer in
#        "B"|"[B")
...
#        ;;
#        "")
#        echo ${menu[$choice]} > /workdata/XCSR/others/gid_change/tmp/answer.tmp
#        return
#        ;;
#    esac
#done
...
if ($#argv == 0) then
    ...
    id | sed -e "s/)/)\n/g" -e "s/id=/id:/g" -e "s/=/\n/g" -e "s/,//g" | sed -e 's/^[ \t]*//g' | grep -v -e "id" -e "groups" -e "所属グループ" -e '^$' | sort -r > $gid_install_dir/tmp/menu.tmp
    
    bash $gid_install_dir/gid_func_menu.sh
    set enter_gid = `cat $gid_install_dir/tmp/answer.tmp | awk '{print substr($0, index($0, "("), index($0, ")") -1 )}' | sed -e "s/(//g" -e "s/)//g"`
    if ("$enter_gid" == "") then
        rm -f $gid_install_dir/tmp/menu.tmp $gid_install_dir/tmp/answer.tmp >& /dev/null
        exit(0)
    endif
    ...
    if ($sel_dir == 0) then
        ...
        ls -la /workdata/ | grep -v -e "root root" -e "users" | sed -e "s/    / /g" -e "s/   / /g" -e "s/  / /g"  | grep -e "$enter_gid" | cut -d " " -f 10 >  $gid_install_dir/tmp/menu.tmp
        
        bash $gid_install_dir/gid_func_menu.sh
        set enter = `cat $gid_install_dir/tmp/answer.tmp`
        if ($enter == "quit") then
            rm -f $gid_install_dir/tmp/menu.tmp $gid_install_dir/tmp/answer.tmp >& /dev/null
            exit(0)
        endif
    ...
    tree -L 3 -d -f | sed -e '/director/d' -e '/^$/d'|grep -e "composer" | sed -e 's/ //g' -e 's/--/@@/g' -e 's/|//g' | cut -c 3- | sed -e 's/|//g'  -e 's/@//g'  > $gid_install_dir/tmp/menu.tmp
    set composer_count = `cat $gid_install_dir/tmp/menu.tmp|wc -l`
...
if($#argv == 1) then
    if(-f $gid_install_dir/tmp/newgrp_flag.tmp && "$argv[1]" == "-b") then
        if (`cat $gid_install_dir/tmp/newgrp_flag.tmp` == 1) then
            rm  -f $gid_install_dir/tmp/newgrp_flag.tmp >& /dev/null
            find -maxdepth 1 -name '*cshrc*' > $gid_install_dir/tmp/menu.tmp
            ...
            if ($enter == "quit") then
                ...
                set cshrc_select = `cat $gid_install_dir/tmp/answer.tmp`
                source $cshrc_select
            endif
        endif
        exit(0)
