...
foreach line ("`cat $filemap`")
    ...
    if ("$file_before" == "-" && "$file_after" != "-") then
        ...
        if ($status == 0) then
            ...
            #diff -y "$path_before/$file_before" "$path_after/$file_after" >> $output_file
            #diff -y --suppress-common-lines "$path_before/$file_before" "$path_after/$file_after" | grep '[|<>]' >> $output_file
            delta  --paging=never --side-by-side --line-numbers --wrap-max-lines=1 --width=200 --hunk-header-style=omit --features=-whitespace-error-highlight --keep-plus-minus-markers  "$path_before/$file_before" "$path_after/$file_after" \
