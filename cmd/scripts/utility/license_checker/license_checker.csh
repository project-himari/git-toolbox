#!/bin/csh -f
#Logic License Check ver.4 (last update 2024/02/21 created by kazama) 
#======================================================#
# User settings
#======================================================#
#Install directory 
set GIT_ROOT = `git rev-parse --show-toplevel`
set INST_DIR = $GIT_ROOT/scripts/utility/license_checker
#Display update interval when using the loop option "-l" (Default:300 [sec])
set interval = 300
#License server setting
set SERVER1 = '5280@scu-proj' #Cadence License Server on the GitBuckect Server(192.168.10.100:8080)
set SERVER2 = 'xxxx@example'


#license token name
set  FEAUTURE_ARRAYS = (Xcelium_Single_Core   Affirma_sim_analysis_env)
#license label          ||                    ||       ||        ||       ||   (Label name is user cuntum)
set  LABEL_ARRAYS    = (Xcelium               simvision)
#license sever          ||                    ||       ||        ||       ||
set  SERVER_ARRAYS   = ($SERVER1              $SERVER1         )
#======================================================#
# Developer settings
#======================================================#
#include file
set subfile_name   = func_menu.sh
set this_file_name = license_checker.csh
...
usage : log_lic -a     -> License all view mode
EOF
