...
choice=0
IFS=$'\n'
menu=($(cat /workdata/XCSR/others/gid_change/gid_menu.tmp))
tail=`expr ${#menu[@]} - 1`
...
while true; do
    ...
    if [ "$answer" = "^[" ]; then
        ...
        echo "quit" > /workdata/XCSR/others/gid_change/gid_answer.tmp
        exit 0
    fi
    case $answer in
        "B"|"[B")
        ...
        ;;
        "")
        echo ${menu[$choice]} > /workdata/XCSR/others/gid_change/gid_answer.tmp
        return
        ;;
    esac
done
