...
set LIC_S2 = '1717@10.20.35.208:1717@tapf01'
set LIC_S3 = '1717@10.20.35.136'  #Eternal Licsense 2014 afs 6token

#License token name
set LIC      = (Virtuoso_ADE_Explorer Virtuoso_ADE_Assembler Virtuoso_Schematic_Editor_L Virtuoso_Multi_mode_Simulation afslicense OASIS_Simulation_Interface  Virtuoso_Layout_Suite_XL  )
#License label  ||                    ||                     ||                          ||                             ||         ||                          || (Label name is user cuntum)
set LIC_L    = (ADE-Explorer          ADE-Assembler          Schematic_Editor            MMSIM                          AFS        OASIS    		       Virtuoso_Layout_XL )	
#License sever  ||                    ||                     ||                          ||                             ||         ||                          ||
set LIC_S    = ($LIC_S1               $LIC_S1                $LIC_S1                     $LIC_S1                        $LIC_S2    $LIC_S1		        $LIC_S1 )
...
#Argument option check (-l or -h or -rm or -w)
    else if (! ("$argv[1]" == "-l" || "$argv[1]" == "-h" || "$argv[1]" == "--help" || "$argv[1]" == "-rm" || "$argv[1]" == "-w")) then
...
#Script mode = 0 => default license view 
    if ($script_mode == 0) then
       ...
       #Delete lines containing specific words 
#        sed -i -e '/lmstat/d' -e '/Users of /d' -e '/mgcld/d' -e '/Analog/d' -e '/Virtuoso/d' -e '/float/d' -e "s/    //g" -e '/^$/d' -e '/endor/d' -e '/lmulti/d' -e 'idc $tmp_file
       # sed -i -e '/lmstat/d' -e '/lmulti/d' -e '/icense/d' -e'/Feature/g' -e '/endor/d' -e "s/    //g" -e '/^$/d'  -e '/Users of /d' -e '/mgcld/d' -e '/Analog/d' -e '/Virtuoso/d' -e '/float/d' $tmp_file
sed -i -e '/lmstat/d' -e '/lmutil/d' -e '/icense/d' -e'/Feature/g' -e '/endor/d' -e "s/    //g" -e '/^$/d' -e '/Users of /d' -e '/mgcld/d' -e '/Analog/d' -e '/Virtuoso/d' -e '/float/d' -e '/cdslmd/d' -e '/saltd/d' $tmp_file
       #bash menu view yobidasi 
        cp $tmp_file $menu_file ; bash $sub_func ; set enter = `cat $answer_file`
        if ("$enter" == "quit") then
    ...
    if ($script_mode < 3) then
        echo "$LINE\n#Status of License(Use/Total)\n$LINE"                                                                    >> $tmp_file
        while ($k <= $#LIC)
            grep -m 1 -e "${LIC[$k]}:" $tmptmp_file  | awk '{ print " " $11"/"$6"	: ";}' | sed -e "s/: /: $LIC_L[$k]/g" >> $tmp_file
            sed -i -e  "0, /${LIC[$k]}:/d" $tmptmp_file 
            sed -n -e "/Users of $LIC[$k]/,/lmstat/p" $tmptmp_file | grep -o -e "$user"                                               >> $tmp_file
            @ k ++
        end
    endif
