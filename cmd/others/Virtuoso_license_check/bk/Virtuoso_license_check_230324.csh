...
#============== Don't touch ==============#
#include file
set subfile_namei  = func_menu.sh
...
set LIC3   = Virtuoso_Multi_mode_Simulation
set LIC4   = Virtuoso_Schematic_Editor_L
set LIC5   = afslicense
set LIC6   = BDA_TOKEN
#set license label
...
EOF
            ...
            if ("$enter" == "quit") then
                ...
                set rm_lic = 3
            else if ("$enter" == "MMSIM") then
                set rm_lic = 4
            else if ("$enter" == "AFS") then
                set rm_lic = 5
            else if ("$enter" == "BDA_ASAKA") then
                set rm_lic = 6 
            endif
            set script_mode = 3
        endif
...
while ($LOOP > 0)
    ...
    lmstat -c $AFS_LICENCE_SERVER     -f afslicense                     >> $tmptmp_file
    lmstat -c $BDA_LICENCE_SERVER     -f BDA_TOKEN                      >> $tmptmp_file
    ...
    set BDA_ASAKA     = `grep "BDA_TOKEN:"                      $tmptmp_file | gawk '{ print $6;}'`
    ...
    set MMSIM_USE     = `grep "Virtuoso_Multi_mode_Simulation:" $tmptmp_file | gawk '{ print $11;}'`
    set AFS_USE       = `grep "afslicense:"                     $tmptmp_file | gawk '{ print $11;}'`
    set BDA_ASAKA_USE = `grep "BDA_TOKEN:"                      $tmptmp_file | gawk '{ print $11;}'`
    ...
    if ($script_mode == 0) then
        ...
        echo "$LINE\n#AFS\n$LINE"                                                    >> $tmp_file
        cat $tmptmp_file | awk '/Users of afslicense:/,/lmstat/'                     >> $tmp_file
        echo "$LINE\n#BDA_ASAKA\n$LINE"                                              >> $tmp_file
        cat $tmptmp_file | awk '/Users of BDA_TOKEN:/,/lmstat/'                      >> $tmp_file
        ...
        if ("$rm_lic" == 1) then
            ...
            echo "lmremove MMSIM            : $MMSIM_USE/$MMSIM (Use/Total)"
            cat $tmptmp_file | awk '/Users of Virtuoso_Multi_mode_Simulation:/,/lmstat/'  > $tmp_file
        else if ("$rm_lic" == 5) then
            echo "lmremove AFS              : $AFS_USE/$AFS (Use/Total)"
            cat $tmptmp_file | awk '/Users of afslicense:/,/lmstat/'                      > $tmp_file
        else if ("$rm_lic" == 6) then
            echo "lmremove BDA_ASAKA        : $BDA_ASAKA_USE/$BDA_ASAKA (Use/Total)"
            cat $tmptmp_file | awk '/Users of BDA_TOKEN:/,/lmstat/'                       > $tmp_file
        endif
        ...
        if ("$rm_lic" == 1) then
            ...
            '{print "lmremove -c " CADENSE_LICENCE_SERVER " Virtuoso_ADE_Explorer " $1 " " $2 " "$3 }'`
        else if ("$rm_lic" == 2) then
            set lm_lic = `grep "" $install_dir/answer.tmp | sed "s/ /,/g" | \
            awk -F, -v CADENSE_LICENCE_SERVER="$CADENSE_LICENCE_SERVER" \
    ...
    if ($script_mode < 3) then
        ...
        grep "BDA_TOKEN:"                       $tmptmp_file | gawk '{ print " BDA_ASAKA        : "$11"/"$6;}'    >> $tmp_file
