...
if ($#argv == 1) then
    if ("$argv[1]" == "-h" ) set script_mode = 0 ;#help view mode
    if ("$argv[1]" == "-l" ) set script_mode = 2 ;#loop mode default interval time
    if ("$argv[1]" == "-rm") set script_mode = 3 ;#license remove mode
    if ("$argv[1]" == "-a" ) set script_mode = 4 ;#license server all view mode
else if ($#argv == 2) then
    #Input Number Judge
    ...
    set interval = "$argv[2]"    
    if ("$argv[1]" == "-l" ) set script_mode = 2 ;#loop mode custum interval time
else if ($#argv > 2) then 
...
if ( $script_mode == 0 ) then
    echo "usage : log_lic        -> The current license usage status is displayed. "
    echo "usage : log_lic -l     -> Keep updating the license display at regular intervals(Loop option)"
    echo "usage : log_lic -l N   -> Set the update interval of Display of loop option to an arbitrary time(N=number)"
    echo "   ex.  log_lic -l 60  -> Display update interval 60 sec"
    echo "usage : log_lic -rm    -> License remove mode"
    echo "usage : log_lic -a     -> License server all view mode"
    exit(0)
endif
#default view mode & common process
if ($script_mode >= 1) then
    ...
    grep -e 'start' -e 'in use' $tmptmp_file \
  | sed  -e "s/Users of /\n$LINE\n#/g" -e "s/ in use)/)\n$LINE\n/g" -e "s/Total of //g"\
         -e "s/ licenses//g"           -e "s/ license//g"           -e "s: issued;:/:g"  > $tmp_file
endif
if ($script_mode == 2) clear
#Command "-rm " : License remove mode
if ($script_mode == 3) then
    if ("$lic_use_arrays[$rm_lic]" == 0) then
        echo "No one uses $LABEL_ARRAYS[$rm_lic] license."
...
#Script mode = 0 => default license view 
        ...
        sed -e 's/^[ \t]*//' -ne "/Users of $FEAUTURE_ARRAYS[$rm_lic]/,/lmstat/p" $tmptmp_file | grep start > $menu_file
        bash $menu_choice $menu_file $menu_answer ;#bash menu_choice view calling 
        if ( `cat $menu_answer` == "quit") then
    ...
    if ($script_mode < 3) then
        ...
        while ($k <= $#FEAUTURE_ARRAYS)
            ...
            sed -n "/Users of $FEAUTURE_ARRAYS[$k]/,/lmstat/p" $tmptmp_file | grep $user       >> $tmp_file
            @ k ++
        end
    endif
    ...
    sed -i -e "s/   $user/==>$user/g"   $tmp_file
